...
import utils
import itertools
from pointnet import PointNet, rotation_6d_to_matrix, PoseDataset
...
# --- Symmetry Utils (Copied/Adapted for Eval) ---
...
def evaluate():
    ...
    for _, row in objects_df.iterrows():
        ...
        # Manual Overrides (Same as in pointnet.py/icp.py)
    ...
    # Load Dataset
    # We use the same PoseDataset but we need to look up object names to get symmetry info
    # The dataset returns obj_id, so we need a map from obj_id to obj_name
    # We can build this map by loading one meta file or hardcoding if we knew the IDs.
    # Better: The dataset loads meta files, we can access them.
    ...
    # We need to map obj_id to name. 
    # We can do this by reading the meta file again or just trusting the dataset to be consistent.
    # Let's read the meta file path from the dataset.
    ...
    for i, batch in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            ...
            pred_R_batch = rotation_6d_to_matrix(pred_rot_6d)
        ...
        for b in range(len(points)):
            ...
            if scene_name != current_scene_name:
                ...
                try:
                    # Visualize every 20th scene (approx 5%)
                    visualize_this_scene = (int(scene_name) % 20 == 0)
                except:
                    visualize_this_scene = False
            ...
            # Compute Metrics
            rot_err = compute_symmetry_aware_loss(pred_R, gt_R_np, sym_str)
            ...
            if rot_err < 20.0 and trans_err < 2.0:
                pass_count += 1
